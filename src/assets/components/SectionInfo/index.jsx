...
export default function SectionInfo({setValues, values, setSection}){
  ...
  const validateInputs = (e)=>{
    ...
    if(values.email !== ""){
      values.email.match(emailRegexp) ? '' : error.email = "this is not email";
    }else{
      error.email = "This field is required";  
    }
    if (values.phone !== "") {
       values.phone.match(phoneRegexp) ? '' : error.phone = "this is not number phone valid";
    }else{
    ...
    return(
        <div className="section_form info">
            ...
            <footer className='footer_info'>
              <button onClick={validateInputs} type='butoon'className='btn_next'>Next Step</button>
            </footer>
